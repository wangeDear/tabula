...
class CouchDBManager {
  constructor() {
    this.config = {
      url: 'http://chromeapp.tabulapp.win/app/tabula',
    ...
    try {
      ...
      const designDoc = {
        ...
        views: {
          ...
          user_settings: {
            map: `function(doc) {
              if (doc.type === 'user_settings') {
                emit(doc.owner, {
                  lastActiveSection: doc.lastActiveSection,
  ...
  async getFavorites(userId = null) {
    ...
    try {
      ...
      console.log('[COUCHDB] User ID:', userId);
      ...
      console.log('[COUCHDB] Parsed favorites:', favorites.length, 'items');
      return favorites;
    } catch (error) {
  ...
  async saveUserSettings(settings) {
    try {
      ...
      if (existingSettings) {
        // 更新现有设置
        settingsDoc._id = existingSettings._id;
        settingsDoc._rev = existingSettings._rev;

        const response = await fetch(`${this.config.url}/${existingSettings._id}`, {
          method: 'PUT',
          headers: this.config.headers,
        ...
        console.log('[COUCHDB] User settings updated successfully');
        ...
        console.log('[COUCHDB] User settings created successfully');
        return await response.json();
      }
    } catch (error) {
      console.error('[COUCHDB] Failed to save user settings:', error);
      throw error;
    }
  }
  ...
  async getUserSettings(userId = null) {
    try {
      ...
      const response = await fetch(`${this.config.url}/_design/favorites/_view/user_settings?key="${userId}"`, {
        method: 'GET',
        headers: this.config.headers
      });
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.log('[COUCHDB] No user settings found');
          return null;
        }
      ...
      if (data.rows && data.rows.length > 0) {
        const settings = data.rows[0].value;
        console.log('[COUCHDB] Retrieved user settings:', settings);
        return {
          _id: data.rows[0].id,
          _rev: settings._rev,
          lastActiveSection: settings.lastActiveSection,
      ...
      console.error('[COUCHDB] Failed to get user settings:', error);
      return null;
    }
  }
